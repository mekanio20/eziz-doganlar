<template>
  <div
    ...
    <nav class="sm:container px-4">
      <div class="flex items-center flex-row py-6 justify-between">
        ...
        <div class="laptop:flex items-center space-x-6 hidden">
          <!-- LANGS -->
          <div class="flex items-center xl:space-x-10">
            <div class="flex items-center space-x-3 cursor-pointer" @click="dropdown()">
              ...
              <div class="relative inline-block">
                ...
                <div v-if="isDropdown" class="absolute bg-white h-fit top-8 right-4 flex flex-col">
  ...
  <!-- Mobile -->
  <div class="select-none w-full fixed z-20 top-0 overflow-hidden duration-200 bg-custom-gradient-1"
    :class="[isBurger ? 'h-full' : 'h-0']">
    <div class="mt-28 px-4">
      <div class="w-full flex flex-col items-start space-y-6 px-1 mt-10">
        <div v-for="item in links" :key="item.id" class="px-2 xl:px-4">
          <router-link :to="item.link" class="text-base font-manrope hover:font-bold duration-100 cursor-pointer pr-4"
            :class="[link === item.link_name ? 'font-bold' : 'font-normal']">
            {{ item.name }}
          </router-link>
        </div>
      ...
      <div v-if="isLang" class="mt-4 w-full h-fit p-2 bg-m_blue_6 flex flex-col">
        <span v-for="(item, index) in langs" :key="index" @click="updateLang(item)" v-show="item !== this.locale"
          class="text-white p-4 cursor-pointer hover:bg-m_bg_blue_2 duration-200">
          {{ item }}
        </span>
      </div>
